"""Package a pre-neutralized liquidity/turnover column into factor Parquet."""
...
def build(arguments: argparse.Namespace) -> Path:
    ...
    output = frame[["date", "asset_code", arguments.value_column]].rename(
        columns={arguments.value_column: "value"}
    )
    ...
    manifest = {
        ...
        "value_semantics": "neutralized_exposure",
        "source_column": arguments.value_column,
    }
    (arguments.output_dir / f"{factor_id}.manifest.json").write_text(
